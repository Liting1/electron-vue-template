...
class Db extends Table {
        ...
        fs.stat(this.options.database, async (err) => {
          ...
          if (err && err.code === 'ENOENT') {
            ...
            try {
                ...
                if (!tables.some((t: any) => t.name === table.name)) {
                  ...
                  if (newField.length) {
                    newField.forEach(
                      ...
                      (item) =>
                        this.db.run(
                          `ALTER TABLE ${table.name} ADD ${item} ${table.data[item]}  ${this.defaultValue(
                            table.initData[item]
                          )}`
                        )
                    );
              ...
              // 删除多余的表格
              const { oldField } = this.utils.diff(
                tables.map((t: any) => t.name),
                this.tableNames.map((t) => t.name)
              );
  ...
  async mergeData(table, data: Record<any, any> = {}, whe = {}) {
    ...
    try {
      ...
      if (res.values().next().value) {
        ...
        this.db.run(
          `UPDATE ${table} SET ${this.formatWhereData({ ...val, ...data }, ', ')} ${whe ? 'where ' + whe : ''};`
        );
      } else {
  ...
  async update(table, data = {}, whe) {
    const UPDATE_SQL = `UPDATE ${table} SET ${this.formatWhereData(data, ', ')} ${
      whe ? 'where ' + this.formatWhereData(whe) : ''
    };`;
